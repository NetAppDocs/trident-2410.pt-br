...
.Passos
. Se você link:../trident-install/kubernetes-deploy-helm.html#deploy-the-trident-operator-and-install-trident-using-helm["Trident instalado usando Helm"], pode usar  `helm upgrade trident netapp-trident/trident-operator --version 100.2410.0` para atualizar em uma única etapa. Se você não adicionou o repositório Helm ou não consegue usá-lo para atualizar:
+
