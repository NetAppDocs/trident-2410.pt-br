...
A força de desprendimento está disponível para `ontap-san` , `ontap-san-economy` e `ontap-nas-economy` apenas.  Antes de habilitar a desconexão forçada, o desligamento não normal do nó (NGNS) deve ser habilitado no cluster do Kubernetes. Para mais informações, consulte link:https://kubernetes.io/docs/concepts/architecture/nodes/#non-graceful-node-shutdown["Kubernetes: Desligamento do nó não gracioso"^] .
